...
# POORLY WORDED NEEDS
def find_element(
    arr: list[int], ranges: list[tuple], rotations: int, index: int
) -> int:
    """
    ...
    for start, end in ranges:
        ...
        rotated_subrange = (
            subrange[-rotations_effective:] + subrange[:-rotations_effective]
        )
...
def extract_even(nested_tuple: tuple) -> tuple:
    ...
    result = tuple(
        ...
        if isinstance(item, int) and item % 2 == 0 or isinstance(item, tuple)
    )
    return result
